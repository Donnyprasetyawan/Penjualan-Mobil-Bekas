...
if(isset($_POST['form1']) ) {
	$valid = 1;
		...
		if(empty($_POST['lama_book'])) {
		} 
    if($valid == 1) {
	    // Updating data
	    if($path!=''):
	    	$statement = $pdo->prepare("UPDATE tbl_booking SET pembayaran=?,nama_bank=?,lama_cicil=?,lama_book=? WHERE id_booking=?");
	    	$statement->execute(array($_POST['pembayaran'],$_POST['nama_bank'],$_POST['lama_cicil'],$_POST['lama_book'],$_REQUEST['id'])); 
		else:
						$statement = $pdo->prepare("UPDATE tbl_booking SET pembayaran=?,nama_bank=?,lama_cicil=?,lama_book=? WHERE id_booking=?");
						$statement->execute(array($_POST['pembayaran'],$_POST['nama_bank'],$_POST['lama_cicil'],$_POST['lama_book'],$_REQUEST['id']));
		endif;
								...
								<form action="" method="post">
									<div class="form-row">
										...
										<div class="form-group col-md-6 col-sm-6 option-item">
											...
											<select data-placeholder="Pilih Item ..." class="form-control chosen-select" name="lama_book">
												<?php
												...
												foreach ($result as $row) {
													?>
													<option></option>
													<option value="<?php echo $row['lama_booking']; ?>" <?php if($row['lama_booking'] == $lambok){echo 'selected';} ?>><?php echo $row['lama_booking']; ?></option>
													<?php
												}
												?>
											</select>
